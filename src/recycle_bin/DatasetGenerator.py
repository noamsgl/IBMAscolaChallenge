...
def U3Dataset(angle_step=9, probability_step=10, shots=1024, save_dir=None):
    ...
    # generate circuits
    for theta, phi, lam in tqdm(itertools.product(np.linspace(0, np.pi, angle_step, endpoint=True), repeat=3)):
        # define circuit
...
for n in range(11, 21, 2):
    angle_step = n
    prob_step = n
    U3Dataset(angle_step=angle_step, probability_step=prob_step,
              save_dir='../datasets/U3_{}_{}.csv'.format(angle_step, prob_step))
